<template>
  <div class="login-container">
      <el-form autoComplete="on" :model="loginForm" :rules="loginRules" ref='loginForm' label-position="left" class="card-box login-form">
          <h3 class="title">vain admin</h3>
          <el-form-item prop="username">
              <span class="svg-container svg-container_login">
                  <svg-icon icon-class="user"/>
              </span>
              <el-input name="username" type="text" v-model="loginForm.username" autoComplete="on" placeholder="用戶名"/>
          </el-form-item>
  ...
  data() {
    ...
    const isValidatePassword = (rule, value, callback) => {
      if (!validatePassword(value)) {
        callback(new Error('请输入数字和字母组成的密码'))
      } else {
        callback()
      }
    }
